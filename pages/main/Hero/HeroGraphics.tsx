...
const TAB_DATA: Record<Tab, {
  ...
  gold: {
    label: "Gold",
    title: "XAU/USD Smart Setup",
    timeframe: "1H",
    price: "$2,320.50",
    entry: "$2,305",
    tp: "$2,345",
    sl: "$2,290",
    alert: "Gold reclaiming demand zone.",
  },
  btc: {
    label: "BTC",
    title: "BTC/USD Momentum Setup",
    timeframe: "4H",
    price: "$64,500",
    entry: "$63,800",
    tp: "$66,200",
    sl: "$62,900",
    alert: "BTC liquidity sweep confirmed.",
  },
  forex: {
    label: "FX",
    title: "EUR/USD Intraday Setup",
    timeframe: "15m",
    price: "1.08520",
    entry: "1.0820",
    tp: "1.0890",
    sl: "1.0790",
    alert: "EUR/USD holding breakout support.",
  },
};

const CANDLES = [
  { wick: "h-14", body: "h-8", color: "bg-rose-500", wickColor: "bg-rose-400" },
  { wick: "h-18", body: "h-10", color: "bg-emerald-500", wickColor: "bg-emerald-400" },
  { wick: "h-12", body: "h-7", color: "bg-rose-500", wickColor: "bg-rose-400" },
  { wick: "h-24", body: "h-14", color: "bg-emerald-500", wickColor: "bg-emerald-400", buy: true },
  { wick: "h-28", body: "h-16", color: "bg-emerald-500", wickColor: "bg-emerald-400" },
  { wick: "h-16", body: "h-9", color: "bg-emerald-500", wickColor: "bg-emerald-400" },
  { wick: "h-20", body: "h-11", color: "bg-emerald-500", wickColor: "bg-emerald-400" },
];

const STRATEGY_ITEMS = [
  { label: "Fast EMA", value: "20", color: "text-emerald-600 dark:text-emerald-400" },
  { label: "Slow EMA", value: "50", color: "text-indigo-600 dark:text-indigo-400" },
  { label: "ATR Vol", value: "14", color: "text-violet-600 dark:text-violet-400" },
  { label: "Session", value: "NY", color: "text-amber-600 dark:text-amber-400" },
];

const VOLUME_BARS = [32, 48, 38, 62, 54, 74];
...
function HeroGraphics() {
  ...
  const [heroTab, setHeroTab] = useState<Tab>("gold");
  const data = TAB_DATA[heroTab];
  ...
  return (
    <motion.div
      ...
      className="relative h-full overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.08)] dark:border-slate-800 dark:bg-slate-950 dark:shadow-[0_20px_80px_rgba(0,0,0,0.45)]"
    >
      {/* Ambient */}
      ...
      <div className="relative flex h-full flex-col p-4">
        {/* Header */}
        <motion.div variants={fadeUp} className="mb-3 flex items-center justify-between gap-3">
          <div className="min-w-0 flex items-center gap-3">
            <motion.div
              whileHover={{ rotate: -8, scale: 1.05 }}
              className="flex h-9 w-9 items-center justify-center rounded-2xl bg-slate-900 text-white shadow-md dark:bg-white dark:text-slate-900"
            >
          ...
          <div className="flex items-center gap-2">
            <div className="hidden sm:flex rounded-2xl border border-slate-200 bg-slate-50 p-1 dark:border-slate-800 dark:bg-slate-900/80">
              {(Object.keys(TAB_DATA) as Tab[]).map((key) => (
                <button
                  key={key}
                  onClick={() => setHeroTab(key)}
                  className="relative rounded-xl px-3 py-1.5 text-[11px] font-semibold transition-colors"
                >
                  {heroTab === key && (
                    <motion.span
                      layoutId="heroTabIndicator"
                      className="absolute inset-0 rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800"
        ...
        <div className="grid flex-1 grid-cols-12 gap-3">
          {/* LEFT SIDE */}
          <motion.div variants={fadeUp} className="col-span-12 lg:col-span-7 flex flex-col gap-3">
            {/* Main chart card */}
              ...
              <div className="relative h-[205px] overflow-hidden rounded-[20px] border border-slate-200 bg-[linear-gradient(to_bottom,rgba(248,250,252,1),rgba(255,255,255,1))] dark:border-slate-800 dark:bg-[linear-gradient(to_bottom,rgba(15,23,42,0.72),rgba(2,6,23,0.95))]">
                ...
                <div className="absolute left-3 right-3 top-3 z-10 flex items-center justify-between">
                  <motion.div
                    ...
                    className="rounded-full border border-slate-200 bg-white/90 px-2.5 py-1 text-[10px] font-medium text-slate-600 shadow-sm dark:border-slate-700 dark:bg-slate-900/90 dark:text-slate-300"
                  >
                ...
                <AnimatePresence mode="wait">
                  <motion.div
                    key={heroTab}
                    className="relative z-10 flex h-full items-end justify-between gap-3 px-5 pb-6 pt-10"
                    initial="hidden"
                    animate="show"
                    exit="hidden"
                      ...
                      >
                        {candle.buy && (
                          <motion.div
                            animate={{ y: [0, -3, 0] }}
                            transition={{ duration: 1.6, repeat: Infinity, ease: "easeInOut" }}
                            className="absolute -top-5 rounded-full bg-emerald-500 px-2 py-0.5 text-[9px] font-bold text-white shadow-md"
                          >
                            BUY
                          </motion.div>
                        )}
                        <div className={`w-[2px] ${candle.wick} ${candle.wickColor} rounded-full`} />
                        <div className={`-mt-2 w-4 ${candle.body} ${candle.color} rounded-md`} />
                      </motion.div>
                    ))}
                  </motion.div>
                </AnimatePresence>
                ...
                <motion.div
                  ...
                  className="absolute bottom-3 right-3 z-20 w-[180px] rounded-[20px] border border-slate-200 bg-white/95 p-3 shadow-xl backdrop-blur dark:border-slate-700 dark:bg-slate-900/95"
                >
            ...
            <div className="rounded-[24px] border border-slate-200 bg-white/80 p-3 shadow-sm backdrop-blur dark:border-slate-800 dark:bg-slate-900/70">
              ...
              <div className="grid grid-cols-12 gap-3">
                ...
                <div className="col-span-4 rounded-[20px] border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-950/60">
                  ...
                  <motion.div
                    className="flex h-[90px] items-end justify-between gap-2"
                    initial="hidden"
                    animate="show"
          ...
          {/* RIGHT SIDE */}
          <motion.div variants={fadeUp} className="col-span-12 lg:col-span-5 flex flex-col gap-3">
            {/* Strategy matrix */}
            <div className="rounded-[24px] border border-slate-200 bg-white/80 p-3 shadow-sm backdrop-blur dark:border-slate-800 dark:bg-slate-900/70">
              <div className="mb-3 flex items-center justify-between">
              ...
              >
                {STRATEGY_ITEMS.map((item) => (
                  ...
                  >
                    ...
                    <div className={`mt-1 text-[12px] font-semibold ${item.color}`}>
                      {item.value}
                    </div>
                  </motion.div>
                ))}
              ...
              <div className="mt-3 border-t border-slate-200 pt-3 dark:border-slate-800">
                <div className="mb-1.5 flex items-center justify-between text-[11px]">
                  <span className="text-slate-500 dark:text-slate-400">Risk meter</span>
            ...
            <div className="grid grid-cols-2 gap-3">
              <motion.div
                whileHover={{ y: -3 }}
                className="rounded-[24px] border border-slate-200 bg-white/80 p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900/70"
              >
                ...
                <div className="mt-1 text-[20px] font-bold tracking-tight text-slate-900 dark:text-white">
                  <AnimatedNumber value={18.4} prefix="+$" suffix="k" decimals={1} />
                </div>
              ...
              >
                ...
                <div className="mt-1 text-[20px] font-bold tracking-tight text-slate-900 dark:text-white">
                  <AnimatedNumber value={74.8} suffix="%" decimals={1} />
                </div>
                ...
                <div className="mt-3 rounded-[18px] border border-blue-200 bg-blue-50 px-3 py-2.5 dark:border-blue-500/20 dark:bg-blue-500/10">
                  ...
                  <AnimatePresence mode="wait">
                    <motion.p
                      ...
                      className="mt-1 text-[10px] leading-5 text-slate-700 dark:text-slate-300"
                    >
                      {data.alert}
                    </motion.p>
                  </AnimatePresence>
            ...
            <div className="grid grid-cols-1 gap-3">
              <motion.div
                variants={fadeUp}
                className="rounded-[22px] border border-slate-200 bg-white/80 px-4 py-3 shadow-sm dark:border-slate-800 dark:bg-slate-900/70"
              >
                <div className="text-[10px] uppercase tracking-wide text-slate-500 dark:text-slate-400">
                  Signal Bias
                </div>
